...
async function loadDRL(){let all=[],loaded=0;for(const url of DRL_SHARDS){try{const r=await fetch(url,{cache:"no-store"});if(!r.ok)continue;const x=await r.json();const rows=Array.isArray(x)?x:(x.rows||[]);all.push(...rows);loaded++}catch(e){}}if(loaded===DRL_SHARDS.length){DATA.drl=all;DRL_STATUS={loaded:true,shards:loaded,rows:all.length,expected:213155};return}DRL_STATUS={loaded:false,shards:loaded,rows:all.length,expected:213155};try{const r=await fetch("data/drl_entries.csv",{cache:"no-store"});if(r.ok){const fallback=parseCSV(await r.text());if(fallback.length){DATA.drl=fallback;DRL_STATUS.rows=fallback.length}}}catch(e){}}
