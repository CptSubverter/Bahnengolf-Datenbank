...
const SOURCES={players:DATA_ROOT+"players.csv",clubs:DATA_ROOT+"clubs.csv",associations:DATA_ROOT+"associations.csv",tournaments:DATA_ROOT+"tournaments.csv",results:DATA_ROOT+"results.csv",rounds:DATA_ROOT+"rounds.csv"};
const DMV_SHARDS=Array.from({length:16},(_,i)=>DATA_ROOT+`dmv_entries_${String(i+1).padStart(2,"0")}.csv`);
...
async function loadCSV(key,url){try{const r=await fetch(url,{cache:"no-store"});if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);const text=await r.text();const rows=parseCSV(text);DATA[key]=rows;if(!rows.length)console.warn("Leere Datenquelle:",key,url)}catch(e){console.error("Datenquelle konnte nicht geladen werden:",key,url,e)}}
async function loadDRL(){let all=[],loaded=0;try{const r=await fetch(DRL_OVERRIDE_URL,{cache:"no-store"});if(r.ok){const x=await r.json();DRL_OVERRIDES=Object.fromEntries((x.overrides||[]).map(v=>[String(v.historical_pass_number),v.player_id]))}}catch(e){console.warn("DRL-Overrides nicht geladen",e)}for(const u of DRL_SHARDS){try{const r=await fetch(u,{cache:"no-store"});if(!r.ok)throw new Error(r.status);const x=await r.json();const rows=Array.isArray(x)?x:(x.rows||[]);if(rows.length){all.push(...rows);loaded++}}catch(e){console.error("DRL-Shard nicht geladen",u,e)}}all.forEach(r=>{if(!r.player_id&&DRL_OVERRIDES[String(r.pass_number)])r.player_id=DRL_OVERRIDES[String(r.pass_number)]});DATA.drl=all;DRL_STATUS={loaded:loaded===8,shards:loaded,rows:all.length,expected:213155}}
async function loadDMV(){const all=[];let loaded=0;for(const u of DMV_SHARDS){try{const r=await fetch(u,{cache:"no-store"});if(!r.ok)throw new Error(r.status);const rows=parseCSV(await r.text());if(rows.length){all.push(...rows);loaded++}}catch(e){console.error("DMV-Shard nicht geladen",u,e)}}DATA.dmv=all;DMV_STATUS={loaded:loaded===16,shards:loaded,rows:all.length,expected:156744}}
