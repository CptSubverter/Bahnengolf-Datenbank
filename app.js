...
const DATA_ROOT="https://raw.githubusercontent.com/CptSubverter/Bahnengolf-Datenbank/main/data/";
...
async function loadDMV(){const rs=await Promise.all(DMV_SHARDS.map(async u=>{try{const r=await fetch(u,{cache:"no-store"});if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);return parseCSV(await r.text())}catch(e){console.error("DMV-Shard konnte nicht geladen werden:",u,e);return[]}}));const all=rs.flat();DATA.dmv=all;const loaded=rs.filter(x=>x.length).length;DMV_STATUS={loaded:loaded===16,shards:loaded,rows:all.length,expected:156744}}
